...
def save_output(content,file_name):
    ...
    f = open(f"./output/text/completed_files.txt", "a")
    f.write(f"{file_name_only}{file_format} \n")
...
def ocr_images():
    ...
    for current_file in images_to_process: # Loops through the images and proccesses them all individually
        ...
        if get_settings.gs('ocr_file_format') == ".md":
            try:
                    ...
                    messages=[
                        {
                            ...
                            'content': 'Convert the document to markdown.', # Prompt
        ...
        if get_settings.gs('ocr_file_format') == ".txt":
            try:
                response: ChatResponse = chat(
                    model=get_settings.gs('ocr_model_name'), # Get the model from the .ini file
                    messages=[
